...
class _ViewState {
  ...
  final List<CancellationRequestModel> requestData;
  final List<CancellationRequestModel> actionItems;
  ...
  _ViewState copyWith({
    ...
    int? tabIndex,
    int? selectedTab,
    List<CancellationRequestModel>? requestData,
    List<CancellationRequestModel>? actionItems,
...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Map<String, String> buildRequestCardData(CancellationRequestModel item) {
  ...
  Future<void> fetchRequestDetailsById(int id) async {
    ...
    try {
      final requests = await cancellationInstance.getRequestsById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await cancellationInstance.getAttachmentsById(id);
      if (attachments != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await cancellationInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await cancellationInstance.getKpiData(
        service.id ?? 0,
        subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await cancellationInstance.getApprovalTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await cancellationInstance
  ...
  Future<void> fetchStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await cancellationInstance.getStatusBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await cancellationInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await cancellationInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchRequests({
    ...
    try {
      ...
      final requests = await cancellationInstance.getRequests(
        offset: 1,
        limit: 8,
  ...
  Future<void> fetchactionItems({
    ...
    try {
      ...
      final items = await cancellationInstance.getActionItems(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await cancellationInstance.uploadAttachments(
          state.attachments,
        );
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await cancellationInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await cancellationInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> onComplete(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      await cancellationInstance.onApprove(payload);
  ...
  Future<void> onSendInProgress(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      // await cancellationInstance.onSendInProgress(payload);
      await Future.delayed(Duration(seconds: 3));
      KAppX.router.pop();
      await fetchactionItems();
      await fetchRequests();
