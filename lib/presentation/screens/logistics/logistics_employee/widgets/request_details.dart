...
class LogisticsRequestDetailsTabScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final stateController = ref.read(_vsLEProvider.notifier);
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!state.isLoading && state.requestDataById.data == null) {
                ...
                children: [
                  ...
                  if (selectedTab == 0)
                    ...
                    WorkflowTabContent(
                      ...
                      details: {
                        'user name':
                            result?.request?.createdByUser?.employeeName,

                        'approver name':
                            approvals?[0].approverUser?.employeeName,
                        'approver id': approvals?[0].approverUser?.id,
                        'approverRole': approvals?[0].approverRole?.name,
                      },
                    ),
                ],
              ),
