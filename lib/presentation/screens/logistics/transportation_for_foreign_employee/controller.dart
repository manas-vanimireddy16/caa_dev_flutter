...
class _ViewState {
  ...
  final List<ForeignEmployeeVehicleRequestModel> requestData;
  final List<ForeignEmployeeVehicleRequestModel> actionItems;
  ...
  _ViewState copyWith({
    ...
    int? tabIndex,
    int? selectedTab,
    List<ForeignEmployeeVehicleRequestModel>? requestData,
    List<ForeignEmployeeVehicleRequestModel>? actionItems,
...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Map<String, String> buildRequestCardData(
    ForeignEmployeeVehicleRequestModel item,
  ) {
  ...
  Map<String, String> buildRequestInformationData() {
    ...
    return {
      ...
      "No of Passengers": request?.numberOfPassengers.toString() ?? 'N/A',
      "Arrival/Departure Date & Time":
          formatDate(request?.arrivalDepartureDatetime) ?? 'N/A',
      "Vehicle Required Location": request?.vehicleRequiredLocation ?? 'N/A',
            ...
            int.tryParse(
              ref.watch(
                    dynamicFormProvider.select(
                      (s) => s.values['no_of_passengers']?.toString() ?? '0',
                    ),
                  ) ??
                  '0',
            ) ??
  ...
  void addPassenger() {
    ...
    state = state.copyWith(passengerNames: updated);
  }

  void removeLastPassenger() {
    if (state.passengerNames.isNotEmpty) {
      final updated = List<String>.from(state.passengerNames)..removeLast();

      state = state.copyWith(passengerNames: updated);
    }
  }

  void updatePassenger(int index, String value) {
    final updated = [...state.passengerNames];
    updated[index] = value;
  ...
  Future<void> fetchRequestDetailsById(int id) async {
    ...
    try {
      final requests = await foreignEmployeeInstance.getRequestsById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchChatById(int id) async {
    try {
      final requests = await foreignEmployeeInstance.getchatById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await foreignEmployeeInstance.getAttachmentsById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await foreignEmployeeInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await foreignEmployeeInstance.getKpiData(
        service.id ?? 0,
        subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await foreignEmployeeInstance.getApprovalTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await foreignEmployeeInstance
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await foreignEmployeeInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await foreignEmployeeInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await foreignEmployeeInstance.uploadAttachments(
          state.attachments,
        );
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await foreignEmployeeInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await foreignEmployeeInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> onVehicleAllocate(Map<String, dynamic> payload) async {
    try {
      ...
      // 3️⃣ Send request
      await foreignEmployeeInstance.onAllocateVehicle(
        payload,
  ...
  Map<String, dynamic> _buildPayload(
    ...
    return {
      ...
      "number_of_passengers": values['no_of_passengers'] ?? 0,

      "purpose_of_travel": values['purpose'] ?? "",

      "vehicle_required_location": values['vehicle_required_location'] ?? "",

      "employee_id_passport": values['employee_passport_number'] ?? "",
      ...
      "arrival_departure_datetime": _buildArrivalDateTime(values),

      "special_instructions": values['special_instructions'] ?? "",

      "passengers": _buildPassengers(state.passengerNames),
  ...
  /// ================= BUILD PASSENGERS =================
  List<Map<String, dynamic>> _buildPassengers(List<String> passengerNames) {
  ...
  /// ================= BUILD DATETIME =================
  String _buildArrivalDateTime(Map<String, dynamic> values) {
    final date = values['arrival_departure_date'];
    final time = values['arrival_departure_time'];
    ...
    try {
      final combined = DateTime.parse("${date.toString()} ${time.toString()}");
  ...
  Future<bool> submitProjectApprovalRequest(
    ...
    try {
      ...
      final response = await foreignEmployeeInstance
