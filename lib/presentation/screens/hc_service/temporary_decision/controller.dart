...
class _VSController extends StateNotifier<_ViewState> {
  ...
  final temporaryDecisionInstance = TemporaryAssignmentDecisionRepoistry();
  ...
  Future<void> fetchRequestDetailsById(int id) async {
    ...
    try {
      final requests = await temporaryDecisionInstance.getRequestsById(id);
  ...
  Future<void> fetchChatById(int id) async {
    try {
      final requests = await temporaryDecisionInstance.getchatById(id);
      if (requests != null) {
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await temporaryDecisionInstance.getAttachmentsById(
        id: id,
      );
      if (attachments != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await temporaryDecisionInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await temporaryDecisionInstance
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await temporaryDecisionInstance
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await temporaryDecisionInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await temporaryDecisionInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<List<TemporaryDecision>> loadMyRequestsPage(
    ...
    if (!mounted) return [];
    try {
      return await temporaryDecisionInstance.getRequests(
  ...
  Future<List<TemporaryDecision>> loadActionItemsPage(
    ...
    if (!mounted) return [];
    try {
      return await temporaryDecisionInstance.getActionItems(
        offset: ListPagination.offsetForPage(pageKey),
        limit: ListPagination.pageSize,
        searchText: searchText,
        status: status,
  ...
  Future<void> fetchRequests({
    ...
    try {
      ...
      final requests = await temporaryDecisionInstance.getRequests(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> fetchactionItems({
    ...
    try {
      final items = await temporaryDecisionInstance.getActionItems(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await temporaryDecisionInstance.uploadAttachments(
          state.attachments,
        );
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await temporaryDecisionInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await temporaryDecisionInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> onComplete(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      await temporaryDecisionInstance.onAssignRejectClose(payload);
  ...
  Future<void> onSendInProgress(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      // await temporaryDecisionInstance.onSendInProgress(payload);
      await Future.delayed(Duration(seconds: 3));
      KAppX.router.pop();
  ...
  Future<void> submitProjectApprovalRequest(
    ...
    try {
      ...
      final response = await temporaryDecisionInstance
