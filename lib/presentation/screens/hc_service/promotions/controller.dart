...
class _VSController extends StateNotifier<_ViewState> {
  ...
  List<DynamicField> get promotionForm => [
    ...
    DynamicField(
      ...
      onChanged: (value, ref) {
        ...
        ref.read(dynamicFormProvider.notifier).autoPopulate({
          ...
          'current_job_title': selectedUser.position?.name,
          'current_salary_grade': selectedUser.grade,
        });
      },
    ),
    ...
    DynamicField(
      ...
      type: FieldType.text,
      required: true,
      placeholder: 'Enter Proposed Job Title',
    ),

    /// -------- CURRENT SALARY GRADE --------
    DynamicField(
      name: 'current_salary_grade',
      label: 'Current Salary Grade',
      type: FieldType.text,
      required: true,
      placeholder: 'Enter Current Salary Grade',
      disabled: true,
    ),

    /// -------- PROPOSED SALARY GRADE --------
    DynamicField(
      name: 'proposed_salary_grade',
      label: 'Proposed Salary Grade',
      type: FieldType.text,
      required: true,
      placeholder: 'Enter Proposed Salary Grade',
    ),
  ...
  Future<void> fetchRequestDetailsById(
    ...
    try {
      final requests = await promotionInstance.getRequestsById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchChatById(int id) async {
    try {
      final requests = await promotionInstance.getchatById(id);
      if (requests != null) {
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await promotionInstance.getAttachmentsById(id);
      if (attachments != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await promotionInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchLocations() async {
    try {
      final locations = await promotionInstance.getLocations();
  ...
  Future<void> fetchKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await promotionInstance.getKpiData(
        service.id ?? 0,
        subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await promotionInstance.getApprovalTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await promotionInstance
  ...
  Future<void> fetchStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await promotionInstance.getStatusBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await promotionInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await promotionInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<List<PromotionsModel>> loadMyRequestsPage(
    ...
    if (!mounted) return [];
    try {
      return await promotionInstance.getRequests(
  ...
  Future<List<PromotionsModel>> loadActionItemsPage(
    int pageKey, {
    String searchText = '',
    String status = '',
  }) async {
    if (!mounted) return [];
    try {
      return await promotionInstance.getActionItems(
        offset: ListPagination.offsetForPage(pageKey),
        limit: ListPagination.pageSize,
        searchText: searchText,
        status: status,
  ...
  Future<void> fetchRequests({
    ...
    try {
      ...
      final requests = await promotionInstance.getRequests(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> fetchActionItems({
    ...
    try {
      ...
      final items = await promotionInstance.getActionItems(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await promotionInstance.uploadAttachments(
          state.attachments,
        );
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await promotionInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await promotionInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> assignEngineer({
    ...
    try {
      ...
      await promotionInstance.onAssignEmployee(payload);
  ...
  Future<void> submitRequiredNewResourceRequest(
    ...
    try {
      ...
      // API CALL
      final response = await promotionInstance.sendPromotionsRequest(payload);
