...
class _ViewState {
  ...
  final List<RequiredNewResourceRequestModel> requiredNewResourceRequestData;
  final List<RequiredNewResourceRequestModel> requiredNewResourceActionItems;
  ...
  _ViewState({
    ...
    required this.requiredNewResourceRequestData,
    required this.requiredNewResourceActionItems,
  ...
  _ViewState.init()
    : this(
        ...
        requiredNewResourceRequestData: [],
        requiredNewResourceActionItems: [],
  ...
  _ViewState copyWith({
    ...
    int? tabIndex,
    int? selectedTab,
    List<RequiredNewResourceRequestModel>? requiredNewResourceRequestData,
    List<RequiredNewResourceRequestModel>? requiredNewResourceActionItems,
    ...
    return _ViewState(
      ...
      requiredNewResourceRequestData:
          requiredNewResourceRequestData ?? this.requiredNewResourceRequestData,
      requiredNewResourceActionItems:
          requiredNewResourceActionItems ?? this.requiredNewResourceActionItems,
...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Map<String, String> buildRequestCardData(
    RequiredNewResourceRequestModel item,
  ) {
    ...
    return {
      ...
      'Position to be Filled': item.positionToBeFilled ?? '-',
      'Grade': item.grade?.toString() ?? '-',
      'Role/Title of Resource': item.roleTitle ?? '-',
      'Education Requirement': item.educationRequirements ?? '-',
      'Required Skills / Expertise': item.requiredSkills ?? '-',
      'Years of Experience': item.yearsOfExperience?.toString() ?? '-',
  ...
  final requiredNewResourceInstance = RequiredNewResourceRepository();
  final securityAccessInstance = SecurityAccessRepoistory();

  final assignmentdecisionInstance = AssignmentDecisionRepoistory();

  List<DynamicField> get requiredNewResourceForm => [
    /// -------- POSITION TO BE FILLED --------
    DynamicField(
      name: 'positionToBeFilled',
      label: 'Position to be Filled',
    ...
    DynamicField(
      ...
      options: state.gradeList
          .map(
            (e) => DropdownOption(
              value: e.id.toString(),
              label: e.financialGrade.toString(),
            ),
          )
          .toList(),
    ),

    /// -------- ROLE / TITLE OF RESOURCE --------
    DynamicField(
      name: 'roleTitle',
      label: 'Role / Title of Resource',
      type: FieldType.text,
      required: true,
      placeholder: 'Enter Role / Title of Resource',
    ),
    ...
    DynamicField(
      ...
      type: FieldType.text,
      required: true,
      placeholder: 'Enter Required Skills / Expertise',
    ),
  ...
  Future<void> fetchGradeList() async {
    try {
      final gradeList = await requiredNewResourceInstance.getGradeList();

      if (gradeList != null) {
        state = state.copyWith(gradeList: gradeList);
        // fetchAssignEmployeesList();
      }
    } on ApiException catch (apiError) {
      Fluttertoast.showToast(msg: apiError.message);
  ...
  Future<void> fetchRequestDetailsById(int id) async {
    try {
      final requests = await requiredNewResourceInstance.getRequestsById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchChatById(int id) async {
    try {
      final requests = await requiredNewResourceInstance.getchatById(id);
      if (requests != null) {
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await requiredNewResourceInstance.getAttachmentsById(
        id,
      );
      if (attachments != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await requiredNewResourceInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchLocations() async {
    try {
      final locations = await requiredNewResourceInstance.getLocations();
  ...
  Future<void> fetchKpi() async {
    try {
      final kpis = await requiredNewResourceInstance.getKpiData(
        service.id ?? 0,
        subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    try {
      final data = await requiredNewResourceInstance
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    try {
      final statusBreakdown = await requiredNewResourceInstance
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    try {
      final data = await requiredNewResourceInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    try {
      final kpis = await requiredNewResourceInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchRequests({
    ...
    try {
      ...
      if (isRefresh || searchText.isNotEmpty || status.isNotEmpty) {
        state = state.copyWith(requiredNewResourceRequestData: []);
      }

      final requests = await requiredNewResourceInstance.getRequests(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
      ...
      // No merging needed
      state = state.copyWith(requiredNewResourceRequestData: requests);
    } catch (e) {
      Fluttertoast.showToast(msg: e.toString());
    }
  }
  ...
  Future<void> fetchActionItems({
    ...
    try {
      if (isRefresh || searchText.isNotEmpty || status.isNotEmpty) {
        state = state.copyWith(requiredNewResourceActionItems: []);
      }

      final items = await requiredNewResourceInstance.getActionItems(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
      ...
      // No merging needed
      state = state.copyWith(
        requiredNewResourceActionItems: items,
        isLoading: false,
      );
    } catch (e) {
      state = state.copyWith(isLoading: false);
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await requiredNewResourceInstance
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await requiredNewResourceInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await requiredNewResourceInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> onComplete(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      await requiredNewResourceInstance.onApprove(payload);
  ...
  Future<void> onSendInProgress(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      // await requiredNewResourceInstance.onSendInProgress(payload);
      await Future.delayed(Duration(seconds: 3));
      KAppX.router.pop();
      refreshRequestLists();
    } catch (e) {
  ...
  Future<void> submitRequiredNewResourceRequest(
    int serviceId,
    int subServiceId,
    ...
    try {
      ...
      final payload = {
        ...
        // ================= REQUIRED NEW RESOURCE DETAILS =================
        "position_to_be_filled": values['positionToBeFilled'],
        "grade": values['grade'],
        "role_title": values['roleTitle'],
        "education_requirement": values['educationRequirement'],
      ...
      // API CALL
      final response = await requiredNewResourceInstance
          .sendRequiredNewResourceRequest(payload);
