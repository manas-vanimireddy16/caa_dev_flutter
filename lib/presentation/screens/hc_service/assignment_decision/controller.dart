...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await assignmentdecisionInstance.getAttachmentsById(
        id: id,
      );
      if (attachments != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await assignmentdecisionInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await assignmentdecisionInstance
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await assignmentdecisionInstance
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await assignmentdecisionInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await assignmentdecisionInstance
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await assignmentdecisionInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await assignmentdecisionInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> onSendInProgress(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      // await assignmentdecisionInstance.onSendInProgress(payload);
      await Future.delayed(Duration(seconds: 3));
      KAppX.router.pop();
  ...
  Future<void> submitProjectApprovalRequest(
    ...
    try {
      ...
      // final response = await assignmentdecisionInstance
