...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await transferFromOneJobtoAnotherJobNatureInstance.deleteAttachment(
        attachmentId,
  ...
  Future<List<JobTransferRequestModel>> loadMyRequestsPage(
    int pageKey, {
    String searchText = '',
    String status = '',
  }) async {
    if (!mounted) return [];
    try {
      return await transferFromOneJobtoAnotherJobNatureInstance.getRequests(
  ...
  Future<List<JobTransferRequestModel>> loadActionItemsPage(
    int pageKey, {
    String searchText = '',
    String status = '',
  }) async {
    if (!mounted) return [];
    try {
      return await transferFromOneJobtoAnotherJobNatureInstance.getActionItems(
        offset: ListPagination.offsetForPage(pageKey),
        limit: ListPagination.pageSize,
        searchText: searchText,
        status: status,
