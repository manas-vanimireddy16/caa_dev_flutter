...
class _VSController extends StateNotifier<_ViewState> {
  ...
  List<DynamicField> get annualIncrementForm => [
    ...
    DynamicField(
      ...
      onChanged: (value, ref) {
        ...
        ref.read(dynamicFormProvider.notifier).autoPopulate({
          'employee_name': selectedUser.employeeName,
    ...
    /// -------- ANNUAL PERIODIC ALLOWANCE --------
    DynamicField(
      name: 'annual_periodic_allowance',
      label: 'Annual Periodic Allowance',
  ...
  Future<void> fetchRequestDetailsById(int id) async {
    ...
    try {
      final requests = await annualIncrementInstance.getRequestsById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchChatById(int id) async {
    try {
      final requests = await annualIncrementInstance.getchatById(id);
      if (requests != null) {
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await annualIncrementInstance.getAttachmentsById(id);
      if (attachments != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await annualIncrementInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchLocations() async {
    try {
      final locations = await annualIncrementInstance.getLocations();
  ...
  Future<void> fetchKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await annualIncrementInstance.getKpiData(
        service.id ?? 0,
        subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await annualIncrementInstance.getApprovalTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await annualIncrementInstance
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await annualIncrementInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await annualIncrementInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<List<PromotionsModel>> loadMyRequestsPage(
    ...
    if (!mounted) return [];
    try {
      return await annualIncrementInstance.getRequests(
  ...
  Future<List<PromotionsModel>> loadActionItemsPage(
    ...
    if (!mounted) return [];
    try {
      return await annualIncrementInstance.getActionItems(
        offset: ListPagination.offsetForPage(pageKey),
        limit: ListPagination.pageSize,
        searchText: searchText,
        status: status,
  ...
  Future<void> fetchRequests({
    ...
    try {
      ...
      final requests = await annualIncrementInstance.getRequests(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> fetchActionItems({
    ...
    try {
      ...
      final items = await annualIncrementInstance.getActionItems(
        offset: 1,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await annualIncrementInstance.uploadAttachments(
          state.attachments,
        );
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await annualIncrementInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await annualIncrementInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> assignEngineer({
    ...
    try {
      ...
      await annualIncrementInstance.onAssignEmployee(payload);
  ...
  Future<void> submitAnnualIncrementRequest(
    int serviceId,
    int subServiceId,
    ...
    try {
      ...
      // API CALL
      final response = await annualIncrementInstance.sendAnnualIncrementRequest(
        payload,
      );
