...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await ImportExportMaterialInstance.deleteAttachment(
        attachmentId,
  ...
  Future<List<ImportExportMaterialModel>> loadMyRequestsPage(
    int pageKey, {
    String searchText = '',
    String status = '',
  }) async {
    ...
    try {
      return await ImportExportMaterialInstance.getRequests(
  ...
  Future<List<ImportExportMaterialModel>> loadActionItemsPage(
    int pageKey, {
    String searchText = '',
    String status = '',
  }) async {
    ...
    try {
      return await ImportExportMaterialInstance.getActionItems(
        offset: ListPagination.offsetForPage(pageKey),
        limit: ListPagination.pageSize,
        searchText: searchText,
        status: status,
  ...
  Map<String, dynamic> _buildPayload(
    ...
    return {
      ...
      "request_date": values['date_of_request'] ?? "",
      "request_type": values['type_of_request'] ?? "",
      "material_movement_date":
          values['requested_material_movement_date'] ?? "",

      "purpose": values['purpose_of_import_export'] ?? "",
      "material_description": values['material_name_description'] ?? "",

      "transport_mode": values['mode_of_transport'] ?? "",
      "description": values['description'] ?? "",

      "hazardous_details": values['hazardous_sensitive_material'] ?? "",
