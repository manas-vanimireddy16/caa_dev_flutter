...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Map<String, String> buildRequestCardData(
    ...
    return {
      ...
      'Requested By': item.dateOfEvent ?? '-',
      'Number of Participants': item.noOfParticipants?.toString() ?? '-',
      'Proposed Implementation Date': item.proposedImplementationDate ?? '-',
  ...
  final annualTrainingPlanInstance = AnnualTrainingPlanRepository();
  final securityAccessInstance = SecurityAccessRepoistory();
  List<DynamicField> get annualTrainingPlanForm => [
    /// -------- NAME OF COURSE --------
    DynamicField(
      name: 'courseName',
  ...
  Future<void> fetchRequestDetailsById(int id) async {
    try {
      final requests = await annualTrainingPlanInstance.getRequestsById(
        id: id,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchChatById(int id) async {
    try {
      final requests = await annualTrainingPlanInstance.getchatById(id);
      if (requests != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await annualTrainingPlanInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchKpi() async {
    try {
      final kpis = await annualTrainingPlanInstance.getKpiData(
        service.id ?? 0,
        subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    try {
      final data = await annualTrainingPlanInstance
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    try {
      final statusBreakdown = await annualTrainingPlanInstance
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    try {
      final data = await annualTrainingPlanInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    try {
      final kpis = await annualTrainingPlanInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchRequests({
    ...
    try {
      ...
      final requests = await annualTrainingPlanInstance.getRequests(
        offset: 0,
        limit: 8,
  ...
  Future<void> fetchActionItems({
    ...
    try {
      ...
      final items = await annualTrainingPlanInstance.getActionItems(
        offset: 0,
        limit: 8,
        searchText: searchText,
        status: status,
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await annualTrainingPlanInstance
      ...
      if (!hasMessage && hasAttachment) {
        ...
        // await annualTrainingPlanInstance.sendAttachment(payload, requestId);
      }
      ...
      if (hasMessage) {
        ...
        await annualTrainingPlanInstance.sendChat(payload, requestId);
      }
  ...
  Future<void> onComplete(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      await annualTrainingPlanInstance.onApprove(payload);
  ...
  Future<void> onSendInProgress(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      // await annualTrainingPlanInstance.onSendInProgress(payload);
      await Future.delayed(Duration(seconds: 3));
      KAppX.router.pop();
      await fetchActionItems();
      await fetchRequests();
  ...
  Future<void> submitAnnualPlanRequest(
    int serviceId,
    int subServiceId,
    ...
    try {
      ...
      final payload = {
        ...
        "no_of_participants":
            int.tryParse(values['numberOfParticipants']?.toString() ?? '0') ??
            0,

        "course_cost":
            double.tryParse(values['courseCost']?.toString() ?? '0') ?? 0,

        "total_cost":
            double.tryParse(values['totalCost']?.toString() ?? '0') ?? 0,

        "proposed_implementation_date":
            values['proposedImplementationDate'], // yyyy-MM-dd

        "description": values['description'],
        "reason": values['reason'],

        // ================= LOCATION =================
        "place": values['place'], // Inside / Outside
        "location": values['location'] ?? "",

        // ================= ATTACHMENTS =================
        "attachments": attachments, // List<Map<String, dynamic>> or files
      };

      debugPrint("✅ Final Payload: $payload");

      // API CALL
      await annualTrainingPlanInstance.sendAnnualTrainingPlanRequest(payload);
