...
class _ViewState {
  ...
  final List<RequestTrainingModel> requestData;
  final List<RequestTrainingModel> actionItems;
  final List<CourseName> courseandParticipants;
  final Map<String, List<String>> courseParticipantsMap;
  final Map<int, String> selectedParticipants;
  ...
  _ViewState({
    ...
    required this.courseandParticipants,
    required this.courseParticipantsMap,
  ...
  _ViewState.init()
    : this(
        ...
        attachmentsById: [],
        courseandParticipants: [],
        courseParticipantsMap: {},
  ...
  _ViewState copyWith({
    ...
    List<RequestTrainingModel>? requestData,
    List<RequestTrainingModel>? actionItems,
    ...
    List<CourseName>? courseandParticipants,
    Map<String, List<String>>? courseParticipantsMap,
    Map<int, String>? selectedParticipants,
  }) {
    return _ViewState(
      ...
      courseandParticipants:
          courseandParticipants ?? this.courseandParticipants,
      courseParticipantsMap:
          courseParticipantsMap ?? this.courseParticipantsMap,
...
class _VSController extends StateNotifier<_ViewState> {
  ...
  Map<String, String> buildRequestCardData(RequestTrainingModel item) {
    ...
    return {
      ...
      "Type Of Training": item.typeOfTraining ?? '-',
      "Type Of Category": item.typeOfCategory ?? '-',
  ...
  Map<String, String> buildRequestInformationData() {
    ...
    return {
      ...
      'Name of Course': request?.courseName ?? 'N/A',
  ...
  final requestTrainingInstance = RequestTrainingRepository();
  final residentalUnitRentalInstance = ResidentalUnitRentalRepository();
  List<DynamicField> get requestTrainingFields => [
    /// ================= TYPE OF TRAINING =================
    DynamicField(
      ...
      required: true,
      initialValue: 'Inside Training',
      options: const ['Inside Training', 'Outside Training'],
    ),
    ...
    DynamicField(
      ...
      options: const ['Planned', 'Out of planned'],
      onChanged: (value, ref) {
        final notifier = ref.read(dynamicFormProvider.notifier);
        if (value == 'Out of planned') {
          notifier.updateValue('course_name', '');
          notifier.updateValue('attendees', []);
        }
      },
    ),
    ...
    DynamicField(
      ...
      /// ⭐ HIDE WHEN Out of planned
      visibleWhen: (values) => values['type_of_category'] != 'Out of planned',

      /// ⭐ COURSE DROPDOWN VALUES
      options: state.courseParticipantsMap.keys
          .map((course) => DropdownOption(value: course, label: course))
          .toList(),

      /// ⭐ ON COURSE SELECT
      onChanged: (value, ref) {
        ref.read(dynamicFormProvider.notifier).updateValue('attendees', []);
        if (value != null) {
          ref
              .read(_vsProvider(params).notifier)
              .onCourseSelected(value.toString());
        }
      },
    ),
    ...
    DynamicField(
      ...
      /// ⭐ ATTENDEES DROPDOWN VALUES
      options: state.selectedParticipants.entries
          .map(
            (participant) => DropdownOption(
    ...
    DynamicField(
      ...
      required: true,
      disabledWhen: (values) => values['type_of_category'] == 'Planned',
      placeholder: 'Enter',
    ),

    /// ================= MEDIA COVERAGE =================
    DynamicField(
      name: 'media_coverage_required',
      label: 'Media Coverage Required',
      type: FieldType.radio,
      required: true,
    ...
    DynamicField(
      ...
      maxFiles: 5,
      maxFileSizeInMB: 10,
      allowedExtensions: const ['doc', 'docx', 'pdf', 'jpg', 'jpeg', 'png'],
    ),
  ];
  ...
  Future<void> fetchChatById(int id) async {
    try {
      final requests = await requestTrainingInstance.getchatById(id);
      if (requests != null) {
  ...
  Future<void> fetchAttachmentsById(int id) async {
    try {
      final attachments = await requestTrainingInstance.getAttachmentsById(id);
      if (attachments != null) {
  ...
  Future<void> deleteAttachment(int attachmentId, {int? requestId}) async {
    ...
    try {
      ...
      await requestTrainingInstance.deleteAttachment(
        attachmentId,
  ...
  Future<void> fetchKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await requestTrainingInstance.getKpiData(
        service.id ?? 0,
        subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await requestTrainingInstance.getApprovalTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalStatusBreakdown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final statusBreakdown = await requestTrainingInstance
  ...
  Future<void> fetchTrendBreakDown(String period) async {
    state = state.copyWith(isLoading: true);
    try {
      final data = await requestTrainingInstance.getTrendBreakdownData(
        period: period,
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchApprovalKpi() async {
    state = state.copyWith(isLoading: true);
    try {
      final kpis = await requestTrainingInstance.getApprovalKpiData(
        serviceId: service.id ?? 0,
        subServiceId: subService.id ?? 0,
      );
  ...
  Future<void> fetchCourseandParticipants() async {
    state = state.copyWith(isLoading: true);

    try {
      final courseName = await requestTrainingInstance
          .getCourseandParticipants();

      /// ⭐ CREATE MAP
      Map<String, List<String>> tempMap = {};

      for (var item in courseName) {
        final course = item.courseName ?? '';

        final participants = item.participants?.nameOfParticipants ?? [];

        tempMap[course] = participants;
      }

      state = state.copyWith(
        courseandParticipants: courseName,
        courseParticipantsMap: tempMap,
        isLoading: false,
      );
    } catch (e) {
      state = state.copyWith(isLoading: false);
  ...
  Future<List<RequestTrainingModel>> loadMyRequestsPage(
    int pageKey, {
    String searchText = '',
    String status = '',
  }) async {
    ...
    try {
      return await requestTrainingInstance.getRequests(
  ...
  Future<List<RequestTrainingModel>> loadActionItemsPage(
    int pageKey, {
    String searchText = '',
    String status = '',
  }) async {
    ...
    try {
      return await requestTrainingInstance.getActionItems(
        offset: ListPagination.offsetForPage(pageKey),
        limit: ListPagination.pageSize,
        searchText: searchText,
        status: status,
  ...
  Future<void> sendChatMessage({
    ...
    try {
      ...
      if (hasAttachment) {
        ...
        final uploadedFiles = await requestTrainingInstance.uploadAttachments(
          state.attachments,
        );
      ...
      if (!hasMessage && hasAttachment) {
        ...
        await requestTrainingInstance.sendAttachment(payload, requestId);
      }
  ...
  Future<void> onComplete(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      await requestTrainingInstance.onApprove(payload);
  ...
  Future<void> onSendInProgress(int approverId, int requestId) async {
    try {
      ...
      // 3️⃣ Send request
      // await requestTrainingInstance.onSendInProgress(payload);
      await Future.delayed(Duration(seconds: 3));
      KAppX.router.pop();
      refreshRequestLists();
    } catch (e) {
  ...
  void onCourseSelected(String courseName) {
    final participants = state.courseParticipantsMap[courseName] ?? [];

    /// ⭐ CREATE MAP<int, String>
    /// 1 -> participant name
    /// 2 -> participant name

    final Map<int, String> participantsMap = {};

    for (int i = 0; i < participants.length; i++) {
      participantsMap[i + 1] = participants[i];
    }

    state = state.copyWith(selectedParticipants: participantsMap);
  }
  ...
  Map<String, dynamic> _buildPayload(
    ...
    return {
      ...
      "req_user_department_id":
          userInfo?.data?.department?.id?.toString() ?? "",
      ...
      "type_of_category": values['type_of_category'] ?? "",
      ...
      /// ⭐ ATTENDEES
      "no_of_attendees": values['number_of_attendees'] ?? 0,

      "name_of_attendees_selection": mapToUsername(
        List<String>.from((values['attendees'] ?? []).map((e) => e.toString())),
      ),

      /// ⭐ MEDIA
      "media_coverage_required": values['media_coverage_required'] ?? "",
  ...
  List<Map<String, dynamic>> mapToUsername(List<dynamic> ids) {
    return ids.map((id) {
      final participantId = int.tryParse(id.toString());

      final participantName = state.selectedParticipants[participantId];

      return {"user_id": participantId, "name": participantName ?? ""};
    }).toList();
  }
  ...
  Future<void> submitProjectApprovalRequest(
    ...
    try {
      ...
      final response = await requestTrainingInstance
          .requestTrainingCreateRequest(payload);
